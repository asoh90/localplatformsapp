...
def append_rates_to_push(brand, provider_element_id, partner_id, price, price_type, rates_to_push_list):
    ...
    try:
        ...
        temp_rates_to_push["RateLevel"] = CUSTOM_TAXO_RATE_LEVEL
    ...
    for segment_id in segment_id_list:
        ...
        if output == "OK":
            if function == "Add":
                ...
                elif not parent_segment_id in rates_created_list:
                    rates_to_push_list, rate_output = append_rates_to_push(brand, segment_id, seat_id, price, price_type, rates_to_push_list)
                    rates_created_list[segment_id] = None
                    if "api_error" in rate_output:
                        rates_output_dict[row_num] = rate_output["api_error"]
...
def read_file_to_retrieve_custom_segments(file_path):
    ...
    for row in segment_json['Result']:
        ...
        # loop to get full segment name
        display_full_name = get_full_segment_name(parent_element_id, display_name, segment_dict)
...
def get_segments_rates(auth_code, partner_id, segment_id):
    rates_dict = {}
    ...
    while result_count > 0:
        result_count, rates_dict = get_rates(auth_code, EYEOTA_BRAND_ID, page_start_index, partner_id, rates_dict, segment_id)
        page_start_index += 1
    ...
    page_start_index = 0
    while result_count > 0:
        result_count, rates_dict = get_rates(auth_code, BOMBORA_BRAND_ID, page_start_index, partner_id, rates_dict, segment_id)
        page_start_index += 1

    return rates_dict


def get_rates(auth_code, brand_id, page_start_index, partner_id, rates_dict, segment_id):
...
def processJsonOutput(auth_code, json_output, function):
    ...
    rates_dict = get_segments_rates(auth_code, None, None)
