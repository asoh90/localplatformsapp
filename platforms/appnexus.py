...
def callAPI(function, file_path):
    ...
    if (function == "Query Segments"):
        output = getQuerySegments()
    elif (function == "Add Segments"):
        output = read_file_to_add(file_path)
...
def read_file_to_add(file_path):
    read_df = None
    try:
        # Skip row 2 ([1]) tha indicates if field is mandatory or not
        read_df = pd.read_excel(file_path, sheet_name="AppNexus Add_Edit", skiprows=[1])
    except:
    ...
    for i in range(len(code_list)):
        ...
        add_response = add_segment(current_code, current_duration, current_segment_name, current_price, current_state)
        ...
        if not current_is_public and not current_segment_id == None:
            # Buyeer member does not already have a list
    ...
    return write_excel.write(write_df, file_name + "_add_segments.xlsx")

def add_segment(code, duration, segment_name, price, state):
    segment_to_add = {
                        "code":str(code),
...
def read_file_to_retrieve_segment_id(file_path):
    ...
    for row_num in range(len(code_list)):
        ...
        retrieved_segment_id = retrieve_segment_id(current_code)
        write_segment_id_list.append(retrieved_segment_id)
        write_member_id_list.append(MEMBER_ID)
    ...
    return write_excel.write(write_df, file_name + "_retrieve_segment_ids.xlsx")
...
def read_file_to_add_existing_segments_to_buyer_member(file_path):
    ...
    return write_excel.write(write_df, file_name + "_add_to_buyer.xlsx")
...
def read_file_to_retrieve_buyer_member_segments(file_path):
    ...
    return write_excel.write(write_df, file_name + "_buyer_segments.xlsx")